...
Read a SINGLE file: `.opencode/cosca/memory/context/cognitive-state.md`
...
> ⚡ **FAST PATH**: If Phase 0.5 loaded cognitive-state.md successfully, skip full memory scan. Only verify the counts from cognitive-state match reality (run `find .opencode/cosca/memory -type f | wc -l` — single command, <1s).

### 3.1 Quick Verify (Fast Path)
```bash
# Verify cognitive-state counts without scanning all files
find .opencode/cosca/memory -type f 2>/dev/null | wc -l
```

...
|-----------|-----------|-------------|-------------|-----------------|
| **Agent Memory** | FileStore (JSON filesystem) | `.opencode/cosca/memory/agent/` | ❌ Plaintext | ⚠️ Baixo — agent names, code references, occasional email/username in learnings |
...
| **Session Context** | FileStore | `.opencode/cosca/memory/session/` | ❌ Plaintext | ⚠️ Baixo — atualmente vazio |
...
- `.opencode/cosca/SECURITY_ARCHITECTURE.md` — Security framework
...
- `.opencode/cosca/memory/agent/cosca-security/learnings.md` — Security agent findings

# Cosca Diagrams

> **Version**: 1.0.0 | **Status**: active | **Owner**: Documentation Chief | **Last Updated**: 2026-07-23

## Available Diagrams

| Diagram | File | Format | Description |
|---------|------|--------|-------------|
| Organizational Hierarchy | [organizational-hierarchy.md](organizational-hierarchy.md) | Mermaid | 40 chiefs org chart |
| Decision Flow | [decision-flow.md](decision-flow.md) | Mermaid | Governance decision process |
| Execution Pipeline | [execution-pipeline.md](execution-pipeline.md) | Mermaid | Runtime execution flow |

## Usage

These diagrams are written in Mermaid format and can be rendered in:
- **GitHub**: Native Mermaid support in .md files
- **GitLab**: Native Mermaid support
- **VS Code**: Mermaid Preview extension
- **Mermaid Live Editor**: https://mermaid.live

## Related
- [Documentation Chief](../departments/documentation/SKILL.md)
- [KERNEL.md](../KERNEL.md)
- [COUNCILS.md](../councils/COUNCILS.md)
- [ORGCHART.md](../company/ORGCHART.md)

...
The command creates the immutable block (`blocks/{sha256}.md`), appends the 1-line trigger to `learnings.md`, updates `chain.dat` and regenerates the Merkle root. Protocol: `.opencode/cosca/memory/LEARNING_PROTOCOL.md` (v3.0.0). Never write learning content directly into learnings.md — it is an index of triggers, not a journal.

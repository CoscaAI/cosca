...
NUNCA no cosca root (.cosca do workspace do cosca nem .opencode/cosca).
...
| **`.opencode/cosca/`** | framework do editor (agentes, skills, patterns) | GLOBAL dos devs |
...
**PROIBIDO em sandbox:**
- escrever learnings em `.opencode/cosca/memory/agent/<nome>/learnings.md` (root do framework)
...
ANTES de delegar trabalho a um agent num projeto ativo, o kernel injeta:
  "workspace = <projeto>
   ...
   NUNCA escreva em .opencode/cosca/memory/ nem em qualquer arquivo do cosca root"
```
...
> (devops, documentation, security, testing) foi migrado de `.opencode/cosca/memory/agent/`

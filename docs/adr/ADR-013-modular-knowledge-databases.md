...
> **Status:** FASE C IMPLEMENTADA — split físico ADITIVO (2026-09-01, verificado em código) | **Owner:** cosca-architecture (Architecture Chief) | **Last Updated:** 2026-09-01
> **Revisão atualizada:** as fatias 1 (mecanismos: `internal/modlink`, `internal/vectoragg`, `cosca db check --gate`) E a Fase C (split físico) estão implementadas. `cosca db build` cria os 11 módulos físicos (core/graph/projects/vector-embed-memory/vector-embed-engines/vector-embed-core/vector-code/vector-docs/vector-fallback/vector-opencode/vector-other) como projeções derivadas do knowledge.db — ADITIVO, não toca a fonte, reversível, idempotente (recria do zero). O vetor (~170 MB) é particionado por RESPONSABILIDADE (regra anti-monstro §2.0); todos os módulos < 100 MB (Decisão 1). O gate mede cada partição via glob `vector-*.db`. O agregador (`vectoragg`) lê as partições físicas (validado: 54.610 vetores, 56.006 entities, 50.198 relationships — EXATOS vs fonte). **PENDENTE (Fase D — corte):** o knowledge.db (585 MB) permanece a fonte da verdade operacional; a migração destrutiva (módulos assumem a operação, consumidores reescritos: restcycle/wake/graph adapter/serve/mcp) exige decisão do Don e parada do serve.

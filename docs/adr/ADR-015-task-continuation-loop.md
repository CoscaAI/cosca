...
> **Status:** PARTIAL — F1+F2+F3 implementadas (2026-09-01, verificado em código) | **Owner:** cosca-architecture | **Last Updated:** 2026-09-01
> **Revisão:** F1 (primitiva neutra) em `internal/task` + `internal/orchestrator`; **F2 (persistência)** em `internal/task/sqlstore.go` (SQLStore — TaskRepository concreto em SQLite `tasks.db`, JSON do TaskState, UPSERT idempotente, cópias seguras); **F3 (control plane vivo)**: o bootstrap instancia o TaskOrchestrator no serve (default ON, `COSCA_TASK_ORCHESTRATOR=false` desliga) com `WithRepository(SQLStore)` + `WithPendingResolver` — o "órfão no bootstrap" da auditoria foi fechado. Pending Resolution integrada aos 2 motores (`AgentEngine.MaxTurns` + `orchestration.Executor.MaxToolRounds`). F4-F5 pendentes: Policy/Risk controller assumindo a decisão de continuidade; eventos TASK_CONTINUE publicados no bus (WithEmitter ainda sem emissor concreto).

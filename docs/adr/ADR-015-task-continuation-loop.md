...
> **Status:** PARTIAL — F1 + F2 implementadas (2026-09-01, verificado em código) | **Owner:** cosca-architecture | **Last Updated:** 2026-09-01
> **Revisão:** F1 (primitiva neutra) implementada em `internal/task` + `internal/orchestrator` (TaskContinuationLoop, sem persistência — estado em memória). **F2 (persistência) implementada**: `internal/task/sqlstore.go` (SQLStore — TaskRepository concreto em SQLite `tasks.db`, JSON do TaskState, UPSERT idempotente, cópias seguras via Clone; o adapter que o orquestrador espera via `WithRepository`). Pending Resolution (`internal/pending`) integrada aos 2 motores de execução — `AgentEngine.MaxTurns` e `orchestration.Executor.MaxToolRounds` — ambos inspecionam o estado antes de parar no limite e resolvem pendências implicadas (sem atalho secreto: registram sinal, nunca executam tool direto). Wiring: serve default ON (`COSCA_PENDING_RESOLUTION=false` desliga). F3–F5 pendentes: `internal/orchestrator` (control plane completo do ADR-015) permanece órfão no bootstrap — a resolução conectada diretamente aos motores foi a decisão pragmática do Don+professor; resta o corte (orchestrator instanciado com SQLStore injetado).

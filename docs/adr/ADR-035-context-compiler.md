...
> **Status:** IMPLEMENTED - F1-F6 entregues e ativas no serve (default ON) (2026-09-01) | **Owner:** cosca-architecture + cosca-kernel | **Last Updated:** 2026-09-01
> **Revisão:** implementação completa das fases F1-F6 — `internal/actiondecode` (instruction packet fail-open), `internal/contextcompile` (TASK/STATE/CONSTRAINTS/FACTS/EVIDENCE + `budget.go` orçamento por seção), `internal/contextrouter` (L0/L1/L2 via deliberação), `internal/contextmetrics` (economia → `cost.Record`, ADR-031), `internal/contextpipeline` (integração). Wiring de produção: serve ativa por padrão (`COSCA_CONTEXT_COMPILER=false` desliga, MaxTokens 2048); bootstrap cria o pipeline. Validado por E2E (2026-09-01): runtime sobe, chain válida (61 blocks), serve ON, MCP responde handshake. Fases pós-1 (escala L2 assistida, ciclo de feedback) permanecem como evolução futura.

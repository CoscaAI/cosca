...
> **Status:** IMPLEMENTED — Fase 0 (métrica por execução) (2026-09-01, verificado em código); Fase 1 (cache de resultado version-safe) pendente | **Owner:** cosca-architecture (Architecture Chief) | **Last Updated:** 2026-09-01
> **Revisão:** Fase 0 implementada em `internal/cost` (`UsefulWork()` decompõe knowledge_gain/task_progress/artifact_value/evidence_gain/decision_gain; `Efficiency()` = UsefulWork/tokens; CLI `cosca cost`; MCP `cosca.cost`). O header anterior "NÃO implementada" estava STALE — o código estava à frente do status declarado. Fase 1 (cache de resultado de agente version-safe) ainda não confirmada.

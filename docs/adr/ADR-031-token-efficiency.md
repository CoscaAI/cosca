...
> **Status:** IMPLEMENTED — Fase 0 (métrica por execução) + Fase 1 (ResultCache version-safe) (2026-09-01, verificado em código) | **Owner:** cosca-architecture (Architecture Chief) | **Last Updated:** 2026-09-01
> **Revisão:** Fase 0 implementada em `internal/cost` (`UsefulWork()` decompõe knowledge_gain/task_progress/artifact_value/evidence_gain/decision_gain; `Efficiency()` = UsefulWork/tokens; CLI `cosca cost`; MCP `cosca.cost`). **Fase 1 implementada**: `internal/cost/resultcache.go` — ResultCache chaveado por `(TaskFingerprint, knowledge_snapshot)`; o snapshot (ADR-029, lock.yaml) entra NA CHAVE (nunca no valor) → duas tarefas iguais em versões diferentes de conhecimento NÃO misturam (testado). TTL opcional, `Clear()` pós-reindex, serialização JSON auditável.

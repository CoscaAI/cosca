...
## 7. Atualiza��o � Chain re-assinada com autoridade do Don (2026-09-01)

A ingest�o dos 692 arquivos disparou a Family Chain (fail-closed funcionando). Os blocks 61/62 foram git-anchored (testemunho de imutabilidade � fallback), mas o DON re-assinou pessoalmente: **Block 63 Ed25519, autoridade real** (fator m�quina DPAPI + TTY + consentimento-ao-conte�do). Chain validada: 63 blocks, 2.005 arquivos.

**Regra operacional (aprovada pelo Don):** mudou o embed ? commit ? DON assina com `cosca-check --sign`. O `--sign-auto` � apenas fallback emergencial, sempre seguido do `--sign`. O `--sign` n�o quebra com commits (n�o depende do git HEAD); o git-anchor quebra (li��o: os blocks 61/62 quebraram ap�s commits de docs/gitignore).

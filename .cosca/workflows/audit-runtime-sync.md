...
>
> **Purpose**: Garantir que a documentação do framework (.cosca/) reflete a realidade do projeto runtime. Detecta drift entre docs e código.
...
# Memory files
find .cosca/memory -name "*.md" | wc -l

# Agent directories
ls -d .cosca/memory/agent/cosca-*/ | wc -l

# Memory INDEX files
find .cosca/memory -name "INDEX.md" | wc -l

# Engine count
ls -d .cosca/engines/*/ | wc -l

# Workflow count
ls .cosca/workflows/*.md 2>/dev/null | wc -l

# Department SKILL files
find .cosca/departments -name "SKILL.md" | wc -l
...
# Session context
grep -E "Memory files|Agents:|Engines:|Workflows:" .cosca/memory/context/session.md

# Codebase overview
grep -E "Go files|packages|endpoints|commands|TSX|providers|adapters" .cosca/memory/codebase/overview.md

# INDEX health
grep -A3 "## Health" .cosca/memory/INDEX.md
```

...
>
> **Purpose**: Manter o snapshot canônico do catálogo (`.cosca/catalog.manifest`) sincronizado com a realidade viva do catálogo (agents/skills/engines/departments). O ciclo é generate-and-diff: o snapshot é o CONTRATO versionado; `--check` enforça SÓ novo drift (não os invariantes); `--audit` expõe os 3 invariantes como débito não-bloqueante.
...
| Trigger | Descrição |
|---------|-----------|
| **Pós-commit** | `.githooks/post-commit` / `post-commit.cmd` rodam `cosca gate catalog --check --summary` quando `.cosca/` muda no último commit |
...
- Escreve `.cosca/catalog.manifest` (lista ordenada de INDEX + títulos canônicos).

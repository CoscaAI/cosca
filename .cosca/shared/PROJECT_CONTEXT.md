...
- Codebase map: `.cosca/memory/codebase/overview.md`
- Constitution: `.cosca/CONSTITUTION.md` (v1.1.0, 8 immutable principles)
- Kernel: `.cosca/KERNEL.md`
- **Tool Execution Policy**: `.cosca/shared/TOOL_EXECUTION_POLICY.md` (contrato universal de execução de tools — rigoroso para segurança, tolerante para execução recuperável; TODOS os agentes obedecem)
- Despertar (wake): `cosca despertar` — reads identity/state from knowledge.db
- Memory: `.cosca/memory/agent/cosca-kernel/` (chain + learnings + failures)

# cosca-architecture - learnings.md EDITOR PRE-FIX
...
| **Related** | See .cosca/memory/codebase/overview.md, .cosca/memory/pattern/ |
...
| **Related** | `.cosca/memory/architecture/grpc-server-plan.md`, `api/rest/server.go`, `internal/runtime/runtime.go`, `internal/cli/serve.go` |
...
| **Related** | `.cosca/memory/architecture/streaming-plan.md`, `api/rest/handler/run.go`, `internal/chat/types.go`, `api/middleware/security.go`, `internal/knowledge/knowledge.go`, `internal/workflows/workflows.go` |
...
| **Related** | `docs/adr/ADR-012-2-zone-cofre-kernel.md`, `.cosca/memory/context/cognitive-state.md`, `internal/embed/cosca/{ORACLE_PROTOCOL,MODEL_PROTOCOL,SECURITY_PROTOCOL}.md`, `internal/oracle/{oracle,search}.go`, `internal/secrets/vault.go`, `pkg/cosca/{jail.go,jail_windows.go}`, `deploy/README-WINDOWS.md` |

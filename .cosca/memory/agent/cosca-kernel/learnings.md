...
## 2026-09-02 - FUSAO + CONTRADICAO (peca 2) - evidencia independente, nao media
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Depois do DTO sensorial (peca 1) e da ponte screen->Observation, o professor apontou a proxima peca: fundir evidencias e detectar contradicao. Construi internal/sensor/fusion. |
| **Level** | 4 |
| **Outcome** | success - fusion.go + fusion_test.go (9 testes PASS), commit 60a4f6d9 |
| **Confianca** | 0.94 (build=0, vet=0, 9 testes PASS) |
| **Tags** | #fusion #contradicao #consenso #log-odds #evidencia-independente #referente #predicado #slot #peca-2 #professor #epistemologia-operacional |
| **Related** | internal/sensor/fusion/fusion.go, internal/sensor/fusion/fusion_test.go, internal/sensor/sensor.go |
| **Learned** | 1) **A CORRECAO TECNICA (a que o professor NAO viu no exemplo dele):** nao se pode fundir em UMA confianca evidencias que falam de eixos DIFERENTES. OCR "Las Vegas" (referente), CLIP "destination" (predicado), detector "text region" (presenca) sao 3 dimensoes — juntar em media (0.91+0.84+0.96/3) e o erro que o proprio professor quer evitar. Separei REFERENTE (identidade: o que esta sendo afirmado) de PREDICADO (atributo: a classe). 2) **CONFIANCA DE CONSENSO = COMBINACAO DE EVIDENCIA INDEPENDENTE (log-odds), NUNCA MEDIA.** 2 sensores concordando DEVEM SUBIR (0.91+0.91 -> ~0.98), nao ficar em 0.91. Media DIMINUI a certeza quando ha concordancia — o oposto do correto. logit(p)=log(p/(1-p)), somar logits, sigmoid. Clamp conservador (-10/+10) evita +-Inf. 3) **CONTRADICAO vive entre REFERENTES CONCORRENTES no MESMO SLOT e MESMA MODALIDADE**, nao dentro de um grupo. "Las Vegas" vs "Los Angeles" na mesma regiao = hipoteses mutuamente exclusivas; o kernel NAO escolhe vencedor — sinaliza o conflito (gatilho da peca 3). 4) **Modalidades diferentes (texto vs visual) sao COMPLEMENTARES, nao contradicao** (visoes de dimensoes distintas). 5) **Baixa confianca (< 0.5) = ruido, nao hipotese** — nao disputa, nao conflita. 6) **Support = fontes independentes (sources distintos), nao contagem de frames** — 2 frames do mesmo OCR contam como 1 fonte. Isso evita inflar consenso com o mesmo sensor repetido. 7) **INTERFACE LIMPA**: Result{Consensus []Fused, Contradictions []Contradiction} — o kernel pergunta "quantos sensores dizem a mesma coisa" e a fusao responde consenso/conflito, sem vencedor silencioso. |
| **Next** | (1) Peca (3): GATE de escalacao — subir para o VLM so quando ha contradicao OU confianca insuficiente, nunca "mandar tudo pra IA". (2) Peca (4): Ledger do Estado do Mundo — alimentar o bus existente (Version/fencing anti-stale), nao crear ledger novo. (3) Conectar a fusao ao fluxo real (screen -> Observation -> Fuse -> gate). |

## 2026-09-02 - GATE DE ESCALACAO (peca 3) - VLM no TOPO, nao na base
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Depois da fusao+contradicao, construir o gate que decide QUANDO subir para o VLM. filosofia do professor: o VLM so e chamado quando ha lacuna epistemologica, nunca "mandar tudo pra IA". |
| **Level** | 4 |
| **Outcome** | success - gate.go + gate_test.go (8 testes PASS), commit 1919176f |
| **Confianca** | 0.94 (build=0, vet=0, 8 testes PASS) |
| **Tags** | #gate #escalacao #vlm #lacuna-epistemologica #resolve #escalate #policy #peca-3 #professor #epistemologia-operacional #decisao-explicavel |
| **Related** | internal/sensor/gate/gate.go, internal/sensor/gate/gate_test.go, internal/sensor/fusion/fusion.go |
| **Learned** | 1) **A FRONTEIRA DO GATE:** o VLM nao e oraculo — e componente julgavel que SOBE apenas quando ha LACUNA EPISTEMOLOGICA (contradicao, confianca insuficiente, pedido explicito). Decide() analisa a fusao e retorna resolve/escalate. 2) **5 CRITERIOS DE ESCALADA** (em ordem): (a) pedido explicito sempre escala (jamais negar analise pedida); (b) contradicao no mesmo slot -> escala, kernel nao escolhe vencedor; (c) consenso forte + corroborado -> resolve (nao troca local barato por IA cara); (d) fonte unica INFERRED fraca -> escala (frágil); (e) confianca abaixo do limiar -> escala (resposta nao confiavel). 3) **POLICY como escolha de POLITICA, nao limiar fixo:** MinConsensus=0.70, ForceEscalateOnContradiction=true, MinEvidenceSources=1. Configuravel por instancia, nunca global (o kernel decide o proprio conservative), e o TestPolicyOverride prova que mudar a politica muda a decisao. 4) **VERDICT EXPLICAVEL (principio 2):** Verdic{Action, Reason, Confidence, Escalate} — o gate sempre diz POR QUE resolveu/escalou. Toda decisao se justifica. 5) **LIÇÃO DE CHAVE (bug de construcao):** o referente da fusao deriva do CONTEUDO normalizado (lowercase/trim), entao o chamador do Decide deve normalizar a MESMA forma. Dois testes falharam por mismatch de caixa ("Las Vegas" vs "las vegas") — o helper ref() precisa espelhar o defaultReferent da fusao. Acoplamento de normalizacao entre packages é frágil; idealmente a chave de referencia deveria ser um tipo/servico compartilhado. 6) **PROBLEMA GIT/PS PERSISTENTE:** mensagens de commit com em-dash (—) ou aspas duplas quebram o parsing do PowerShell (pathspec). Solucao: escrever a mensagem num arquivo temp e usar git commit -F. |
| **Next** | (1) Peca (4): Ledger do Estado do Mundo — alimentar o bus existente (internal/perception/bus, Version/fencing anti-stale), nao criar ledger novo do zero. (2) EXTRAIR a normalizacao de referencia num tipo compartilhado (evita o acoplamento frágil gate<->fusion). (3) Conectar o fluxo completo: screen -> Observation -> Fuse -> gate -> (resolve | escala VLM). |

## 2026-09-03 - FAMILY CHAIN: editar arquivo vigiado exige re-assinatura (lição dura)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Migrar 3 linhas legadas do learnings.md do kernel (L9/L10/L11, formato antigo -> canônico P15) para zerar o guarda de memória. Edição foi correta, mas alterou arquivo vigiado. |
| **Level** | 3 |
| **Outcome** | success - guarda zerado, mas a edição quebrou a family chain (Block 63 TAMPERED) e travou o boot do serve |
| **Confianca** | 0.92 (verificado com cosca-check -root .) |
| **Tags** | #family-chain #integridade #cosca-check #serve #fail-closed #block-63 #licha-espinhosa #licao |
| **Related** | .cosca/family_chain.dat, internal/embed/cosca/memory/agent/cosca-kernel/learnings.md, cosca-check --sign |
| **Learned** | 1) A family chain (muralha de integridade Ed25519, machine-bound) vigia os learnings.md de TODOS os agentes em internal/embed/cosca/memory/agent/*. Editar qualquer um deles sem re-assinar quebra a assinatura do bloco. 2) O bootstrap do serve faz fail-closed: valida a chain no boot e, ao detectar breach, logger.Fatal("family chain breach detected") -> exit 1 SILENCIOSO (o serve nem loga a razão em stdout/stderr, morre quieto após a linha 'Operando em modo determinístico'). 3) DIAGNÓSTICO que economiza horas: ao ver um comando que executa agente (serve, mcp, bridge) morrer com exit 1 SEM mensagem de erro útil, rodar PRIMEIRO `cosca-check -root .` — se vier "FAMILY CHAIN BREACH DETECTED", é o problema. NÃO culpar porta/firewall/jail/MCP (o bridge serve sobe normal porque NÃO passa pelo mesmo path). 4) SOLUÇÃO: re-assinar com `cosca-check --sign` (chave Ed25519 machine-bound, portão do Don). Esse comando NÃO roda via shell automatizado/TTY ausente — exige terminal interativo real (fail-closed de presença). Após re-assinar, chain valid e serve volta a subir. 5) REGRA PARA O FUTURO: antes de editar qualquer arquivo sob internal/embed/cosca/** OU .cosca/family_chain.dat, verificar impacto na chain; se for editar um arquivo vigiado, preparar a re-assinatura autorizada NO MESMO passo, nunca depois. O `memory register` é o caminho que faz isso numa tacada (atualiza learnings.md + chain.dat + merkle) mas também exige presença do Don (TTY). |
| **Next** | (1) Sempre checar `cosca-check -root .` antes de assumir que serve/mcp/bridge falhou por porta/jail. (2) Incluir a verificação da chain como passo pré-edicao em qualquer tarefa que toque arquivos de embed ou da família. (3) Considerar documentar este fluxo no AUTO_EVOLUTION_PROTOCOL ou em um skill de integridade. |

## 2026-09-03 - Baseline FASE 1: benchmark da esteira (smoke) - primeiro run
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Campanha de benchmark da esteira - Fase 1 (baseline). Rodar suite cosmé-esteira-smoke (3 cases) pela 1ª vez para capturar estado atual da esteira real (Planner→StepRunner→RecoveryLoop→DoD). |
| **Level** | 3 |
| **Outcome** | baseline capturado - 3 cases, 0 passou (report cosca-esteira-smoke-20260903-040923.json persistido em .cosca/evals/reports/) |
| **Confianca** | 0.85 (verificado via eval run smoke + report JSON) |
| **Tags** | #esteira #benchmark #baseline #smoke #evals #pipeline #dod #planner |
| **Related** | internal/evals/{runner,metrics,report}.go, internal/embed/cosca/evals/suites/smoke.yml, .cosca/evals/reports/ |
| **Learned** | 1) A esteira FUNCIONA: o Planner planeja via esteira real (plan-feature-development: 19 tasks; plan-bug-fix: 8 tasks) - mecanismo de planejamento saudável, determinístico. 2) Há DUAS causas de falha distintas: (a) falha de VERIFICAÇÃO do harness: `go vet ...` retorna "verify command canceled" - problema de verify/runner, NÃO da esteira; (b) falha REAL de DoD: execute-adhoc-task rejeitou com code_exists/builds/tests_pass REQUIRED falhando (modo determinístico - sem code gerado), enquanto no_regressions/security_ok/documentated passaram. Isso prova que o DoD avalia honestamente e rejeita quando o código não builda. 3) PassRate zero não significa esteira quebrada - significa que (a) os verify commands do harness estão sendo cancelados e (b) o DoD está exigindo code_real que não existe no modo determinístico. 4) AÇÃO PARA FASE 2: investigar por que `go vet` está sendo cancelado (provável timeout/contexto do verify), e decidir se a campanha roda com provider real (ollama) para os checks de execução passarem, ou se o DoD dos cases de benchmark precisa contemplar o modo determinístico. |
| **Next** | (1) Fase 2: rodar suite esteira completa (5 cases) com timeout controlado. (2) Investigar "verify command canceled" no harness de verify (contexto/timeout). (3) Decidir se execução dos cases de feature/bugfix/refactoring usa provider real (ollama) para DoD de code_exists/builds/tests passarem, sob aval do Don (mutação do codebase). (4) Comparar baseline contra runs futuros (track de melhoria). |

## 2026-09-03 - Baseline FASE 2: diagnóstico das falhas do benchmark da esteira
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Fase 2 da campanha de benchmark: diagnosticar a causa raiz das 3 falhas do smoke run (verify command canceled + DoD not fully passed). |
| **Level** | 3 |
| **Outcome** | 2 causas raiz identificadas: (1) harness verify é Linux-only (sh -c); (2) DoD avalia por evidência e rejeita no modo determinístico sem provider |
| **Confianca** | 0.95 (verificado no código verify.go, done.go, e testes t.Skip no Windows) |
| **Tags** | #esteira #benchmark #fase2 #diagnostico #sh-shell #verify #dod #windows #linux-only #cross-platform |
| **Related** | internal/evals/verify.go (linha 34 sh -c), internal/evals/verify_procgroup.go, internal/pipeline/done.go (DoD por evidência), internal/evals/*_test.go (t.Skip Windows) |
| **Learned** | 1) CAUSA RAIZ do "verify command canceled": internal/evals/verify.go usa `exec.CommandContext(vctx, "sh", "-c", cmdline)` - hardcoded em shell POSIX. No Windows NATIVO, `sh` NAO existe no PATH (Go vet roda perfeitamente direto, exit 0), entao o exec falha com context.Canceled. TODOS os testes de internal/evals/*_test.go dao `t.Skip` no Windows por exatamente isso (bug CONHECIDO e aceito): o harness de benchmark é LINUX-ONLY na verificacao. 2) CAUSA RAIZ do "Definition of Done not fully passed": o DoD (internal/pipeline/done.go) avalia por EVIDENCIA REAL (BuildResult/TestResult/OutputFiles via evaluateWithEvidence/evaluateCheck), NAO por shell. No modo deterministico (provider=none), nenhum task gera build/test evidence -> o DoD CORRETAMENTE rejeita code_exists/builds/tests_pass (REQUIRED) enquanto passa no_regressions/security_ok/documentated. NAO e bug - e o DoD fazendo o trabalho certo (sem vacuous true). 3) LIÇÃO: a esteira em si esta saudavel (Planner planeja, DoD valida por evidencia); o GAP de cross-platform esta no harness de verify (sh -c), nao na esteira. 4) Para a campanha gerar pass em cases de execucao, precisa de provider real (ollama) -> o que MUTA o codebase e exige aval do Don. 5) Se o Don quiser benchmark em Windows, o fix seria tornar o RunVerifyCommands cross-platform (usar o shell nativo cmd /c no Windows OU um helper que selecione sh vs cmd vs powershell por GOOS). |
| **Next** | (1) Propor ao Don: (a) rodar cases de execucao com provider real (ollama) - muta codebase; (b) OU fixar RunVerifyCommands para ser cross-platform (cmd /c no Windows) e rodar benchmark determinístico; (c) OU limitar a campanha a plan-only (code-review, security-audit) que nao mutam. (2) Se rodar execucao real, garantir rollback via commit point gold (2c37940f). (3) Registrar qualquer decisao do Don como proximo passo. |

## 2026-09-03 - Campanha FASE 3: execucao real da esteira com provider (feature-development)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Rodar o case feature-development da suite esteira com provider real (ollama/qwen2.5-coder), timeout 600s, para o DoD de code_exists/builds/tests_pass ter chance de passar. |
| **Level** | 3 |
| **Outcome** | esteira executou 19 tasks ponta-a-ponta (3m22s), mas DoD ainda rejeitou code_exists/builds/tests_pass; codebase INTACTO (nenhum .go/.ts criado) |
| **Confianca** | 0.92 (verificado: 19 tasks completed, 0 arquivos de codigo, report cosca-esteira-20260903-041606.json) |
| **Tags** | #esteira #benchmark #fase3 #execucao-real #provider #ollama #dod #code_exists #honestidade |
| **Related** | internal/embed/cosca/evals/suites/esteira.yml, .cosca/evals/reports/cosca-esteira-20260903-041606.json, .cosca/history/PLAN-20260903-*.jsonl, internal/pipeline/done.go |
| **Learned** | 1) A esteira EXECUTOU de ponta a ponta com provider real: 19 tasks (Wizard Intake, Requirement Analysis, Solution Design, Backend/Frontend Implementation, Unit & Integration Tests, QA, Documentation, etc.) todos "completed" em 3m22s - o Planner e o StepRunner FUNCIONAM. 2) MESMO com provider real, o DoD rejeitou code_exists/builds/tests_pass porque a esteira NÃO gerou evidence real: os tasks produziram ANALISE/RELATORIO TEXTUAL (o qwen2.5-coder em modo esteira descreve o pipeline, nao implementa codigo), sem OutputFiles/BuildResult/TestResult. 3) O DoD esta CORRETO e honesto: sem diff/build/test evidence, nao da vacuous pass em code_exists/builds/tests_pass. A esteira "completou" tasks mas nao produziu codigo - e o DoD captou isso. 4) codebase ficou INTACTO (nenhum .go/.ts/.py/.sql criado/modificado), chain valid, serve running - o run nao deixou mutacao. 5) LIÇAO DE CAMPAINHA: a esteira é um ORQUESTRADOR (planeja/executa/valida) - a qualidade da IMPLEMENTAÇÃO depende do provider/agente que executa os tasks. Com qwen2.5-coder o resultado e analitico-textual, nao implementativo. Para benchmark de feature real, seria preciso um modelo/agente que realmente escreva codigo (ou aceitar que o case so passa quando ha evidence). 6) ROLLBACK: nao foi necessario (codebase intacto); o commit point gold 2c37940f e o checkpoint de seguranca. |
| **Next** | (1) Reportar ao Don: a esteira funciona como orquestrador, mas implementacao real depende do provider. (2) Decidir: (a) trocar o provider de execucao para um modelo mais implementativo; (b) ajustar os cases do benchmark para terem verify de relatorio (nao code_exists/builds) quando o provider e analitico; (c) fixar o harness verify para Windows (cmd /c) para o benchmark ser cross-platform. (3) Commit do estado final (codebase intacto + aprendizados). |

## 2026-09-03 - FASE 3: fix cross-platform do harness verify (sh -> cmd.exe via safe.SafeShellExec)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Corrigir o harness de verify do benchmark para ser cross-platform no Windows (opcao 2 aprovada pelo Don). Substituir `sh -c` (Linux-only) por shell nativo. |
| **Level** | 3 |
| **Outcome** | fix implementado e validado - smoke passou de 0/3 para 2/3 (plan-feature-development reward 2.00, plan-bug-fix 1.00; execute-adhoc-task segue falhando por DoD sem code real, nao por bug) |
| **Confianca** | 0.95 (go build ok, go vet ok, go test internal/evals ok, chain valid, smoke re-run 2 passed) |
| **Tags** | #esteira #benchmark #fix #cross-platform #verify #windows #cmd-exe #safeshellexec #fase3 |
| **Related** | internal/evals/verify.go, internal/evals/verify_test.go, internal/safe/shell_exec_{windows,unix}.go, commit e1eb8f94 |
| **Learned** | 1) CAUSA: internal/evals/verify.go usava `exec.CommandContext(vctx, "sh", "-c", cmdline)` - hardcoded em shell POSIX. No Windows nativo `sh` NAO existe, entao o verify falhava com "verify command canceled" (context.Canceled) - os comandos de verif sequer rodavam. 2) FIX APLICADO: substituir por `safe.SafeShellExec` (helper cross-platform ja existente no codebase, em internal/safe). No Windows usa `cmd.exe /c <arquivo-temp>`, no Unix `bash <arquivo-temp>`. Vantagens: (a) funciona no Windows nativo; (b) MAIS SEGURO - escreve o comando num arquivo temp e executa sem interpolacao (evita command injection via `cmd /c <code>` / `bash -c <code>`). 3) TESTES: internal/evals/*_test.go tinham `t.Skip` no Windows assumindo o bug; removi os skips obsoletos de Pass/Fail/Multiple/ContextCanceled (agora rodam no Windows). O teste de Timeout ficou skip no Windows por um bug PRE-EXISTENTE e SEPARADO: o process-tree kill via Job Object/taskkill mata o processo, mas `exec.Cmd.CombinedOutput` nao retorna ate o pipe fechar (o ping orfao segura o wait). 4) IMPORTANTE: editar `internal/evals/` NAO quebra a family chain (que vigia apenas internal/embed/cosca/memory/agent/*/learnings.md) - verificado com cosca-check -root . (chain valid). 5) VALIDACAO FINAL: go build ./... ok, go vet ok, go test ok; smoke re-run: plan-feature-development PASSED (reward 2.00/0.80), plan-bug-fix PASSED (1.00), execute-adhoc-task failed (DoD sem code real - limite do provider analitico, nao bug do harness). Metrics: Accuracy 0.67, Precision 1.0, F1 0.8. |
| **Next** | (1) Tratar o process-tree kill no Windows (teste de Timeout) como bug separado - possivelmente via `cosca bug`, pois bloqueia o benchmark de cases com timeout no Windows. (2) Re-run da suite esteira completa (5 cases) com o fix para baseline cross-platform completo. (3) Se objetivo for case de execucao passar no DoD, trocar provider para modelo implementativo ou ajustar cases para verify de relatorio. |

## 2026-09-03 - Campanha concluida: baseline cross-platform + bug BUG-0001 registrado
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Consolidar a campanha: re-run da suite esteira completa (5 cases) com o fix cross-platform, validar codebase intacto e registrar o bug de process-kill no Windows. |
| **Level** | 3 |
| **Outcome** | suite esteira completa: 2 passed (code-review, security-audit), 3 failed (feature-dev, bug-fix, refactoring - DoD sem code real); BUG-0001 registrado; codebase intacto; chain valid |
| **Confianca** | 0.93 (go build/vet ok, chain valid, reports persitidos) |
| **Tags** | #esteira #benchmark #baseline #cross-platform #bug-0001 #process-kill #windows #conclusao |
| **Related** | internal/embed/cosca/evals/suites/esteira.yml, .cosca/evals/reports/cosca-esteira-20260903-045042.json, cosca bug BUG-0001, internal/pipeline/done.go |
| **Learned** | 1) BASELINE CROSS-PLATFORM FINAL da suite esteira completa (5 cases): code-review PASSED (plan-only, 8 tasks, 2.457s), security-audit PASSED (plan-only, 8 tasks, 1.992s), feature-development FAILED (DoD sem code real), bug-fix FAILED (DoD sem code real), refactoring FAILED (DoD sem code real). Metrics: Accuracy 0.40, Precision 1.0, Recall 0.40, F1 0.57, BootstrapStd 0.22. 2) O fix cross-platform do harness verify resolve POR COMPLETO os cases plan-only (code-review e security-audit agora PASSAM no Windows) - que sao os que dependem do verify. Os cases de EXECUCAO (feature/bugfix/refactoring) falham por causa do DoD sem code real (limite do provider analitico, nao bug). 3) A esteira como ORQUESTRADOR esta validada: Planner planeja (8-19 tasks), StepRunner executa, DoD valida por evidencia honesta. 4) BUG-0001 REGISTRADO (cosca bug register): fingerprint evals.verify:timeout_process_not_killed:internal/evals/verify.go:verify_timeout - o process-tree kill via Job Object/taskkill mata o processo, mas exec.Cmd.CombinedOutput nao retorna ate o pipe fechar no Windows (o subprocesso orfao segura o wait). Isso bloqueia o teste de Timeout no Windows (mantido t.Skip). 5) codebase ficou INTACTO apos o run completo (nenhum .go/.ts/.py/.sql criado) - a esteira nao muta nada, ela orquestra. 6) TODOS os report e aprendizados da campanha persitidos e commitados: 2c37940f (gold), 1900bb7c (fase 1/2), e6cbf5f2 (fase 3 execucao), e1eb8f94 (fix verify), f15a9c02 (aprendizado fase 3). |
| **Next** | (1) BUG-0001 (process-kill Windows) fica PENDENTE - tratar quando for prioridade, envolve mexer no killing do process-tree (processutil_tree_windows.go) ou no uso de CombinedOutput. (2) Se quiser cases de execucao passando no DoD, trocar provider para modelo implementativo (ex: qwen3-coder:30b) OU ajustar os cases de benchmark para verify de relatorio (nao code_exists/builds) quando o provider e analitico. (3) Campanha de benchmark da esteira ENCERRADA - resultados e aprendizados arquivados. |

## 2026-09-03 - CORRECAO DE ROTA: intencao como dado explicito do plano (tasks de acao vs consulta)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Corrigir a rota que fazia tasks de IMPLEMENTACAO serem interceptadas pelo deterministicResponse (knowledge determinístico) antes de chegar ao LLM com tools. Intencao vira dado explicito do TaskNode, decidida no planejamento (Opcao A aprovada pelo professor). |
| **Level** | 4 |
| **Outcome** | rota corrigida: tasks de ACAO agora chegam ao LLM com tools; bug-fix passou de 1/8 tasks ao LLM (antes) para 4 tasks de acao ao LLM (depois). "Fix Design" ESCREVEU fix_approach.md via write_file (prova de que o executor usa tools). |
| **Confianca** | 0.93 (go build ok, go test pipeline/orchestration ok, chain valid, step events evidenciam tools) |
| **Tags** | #rota #intent #deterministicResponse #acao-vs-consulta #opcao-a #roteamento #executor #tools #write_file |
| **Related** | internal/pipeline/{planner,agent_router,runner,steprunner,orch_adapter}.go, internal/cli/pipeline_wiring.go, internal/orchestration/executor.go |
| **Learned** | 1) CAUSA RAIZ (diagnostico): tasks de implementacao usam o TEXTO LITERAL do workflow .md (ex "Implement backend changes"), que esta indexado no knowledge engine. O BuildKnowledge busca esse texto e retorna o PROPRiO workflow com Score 2.17 (auto-referencia/efeito espelho). O deterministicResponse (score >= 0.6) intercepta e ENCERRA antes do LLM. Resultado: 7/8 tasks caíam no "Conhecimento da casa". 2) CORRECAO ARQUITETURAL (Opcao A + exigencia do professor): a intencao da task vira DADO EXPLICITO, nao heuristica no executor. fluxo: DetectTaskType(desc) no workflowToPlan -> TaskNode.IntentType -> RunRequest.IntentType -> orchReq.Context["intent"] -> pc.Data.Extra["intent"] -> executor le o dado. 3) O executor so DECIDE pelo dado: se IsActionIntent(intent) (implement-fix/create-handlers/build-verify/etc) -> pula deterministicResponse e vai ao LLM com tools; tasks de consulta/avaliacao (analyze/review/diagnose/document) continuam deterministicas. NENHUM enfraquecimento do deterministicResponse global. 4) RESULTADO POS-CORRECAO (bug-fix): DOS 4 tasks de acao (Fix Design, Fix Implementation, Regression Tests, Code Review), "Fix Design" ESCREVEU fix_approach.md via write_file (211 bytes) - a tool esta conectada e o modelo a chama. "Fix Implementation" e "Regression Tests" produziram PROSA (descrevem o fix/testes em texto, nao criam codigo). "Code Review" chamou tool "code_review" que NAO EXISTE no registry ("tool not found: code_review") - o modelo inferiu nome de tool errado. 5) DOIS DESCOMPASSOS novos expostos: (a) o OrchAdapter.Run tem um FALLBACK determinístico de template que mascara o erro real do executor ("no template for prompt: ...") - deveria falhar-loud; (b) o StepRunner marca step como Success = BuildResult==nil || Success, mas BuildResult/TestResult vem NULO nos tasks de acao (o modelo nao rodou build/test), entao o DoD nao tem evidence e rejeita code_exists/builds/tests_pass mesmo com steps "completed". 6) LIÇAO: a rota foi corrigida SEM tocar no executor (intencao = dado). O proximo teste sera o LIMPO para julgar o modelo (o "pedreiro"): se qwen3-coder:30b ainda recusar implementar com a rota correta, a culpa e do modelo com evidencia. ANTES vs DEPOIS: ANTES = deterministicResponse interceptava 7/8 (1 task chegava ao LLM); DEPOIS = tasks de acao chegam ao LLM (4 tasks), "Fix Design" usa write_file. |
| **Next** | (1) Desmascarar o erro real do executor: o fallback determinístico de template do OrchAdapter.Run mascara o executor_failed - decidir fail-loud (reportar o erro real do provider) em vez de "no template for prompt". (2) Rodar os 3 cases completos (feature-development, bug-fix, refactoring) com a rota corrigida e medir: tasks ao LLM, tools usadas, write_file/edit, arquivos alterados, testes, DoD. (3) A/B pos-rota com qwen3-coder:30b: teste LIMPO do "pedreiro" - se recusar implementar, culpa do modelo com evidência. (4) Commit da correcao de rota (8 arquivos: planner, agent_router, runner, steprunner, orch_adapter, executor, pipeline_wiring, agent_router_test). |

## 2026-09-03 - A/B LIMPO: qwen2.5-coder vs qwen3-coder:30b (rota corrigida, diagnostico corrigido)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | A/B controlado pos-correcoes (determinismo + mascaramento de erro). Trocar SOMENTE o modelo executor qwen2.5-coder -> qwen3-coder:30b, mantendo harness/tools/schema/DoD/routing/executor/ambiente IDENTICOS e estado dura'vel limpo. Objetivo: medir tool-use agentico do "pedreiro" com o sistema congelado. |
| **Level** | 4 |
| **Outcome** | AMBOS modelos produziram PROSA em vez de tool-call de ESCrita. 30B NAO criou nenhum arquivo .go. 0 write_file/edit_file/create_file executados. Comportamento EQUIVALENTE (nao melhora clara de tool-use). |
| **Confianca** | 0.92 (3 cases rodados com estado limpo, history inspecionado, 0 arquivos criados) |
| **Tags** | #ab #qwen3-coder-30b #tool-use #roteamento #diagnostico #pedreiro #modelo #contrato-tool #schema |
| **Related** | .cosca/evals/reports/cosca-esteira-*.json (baseline), .cosca/history/WF-{bug-fix,feature-development,refactoring}.jsonl (AB 30B), internal/orchestration/executor.go |
| **Learned** | 1) RESULTADO CRITICO: com a rota corrigida E o erro desmascarado, o qwen3-coder:30b (17GB) NO escreveu nenhum codigo - produziu PROSA nos tasks de acao (Fix Design/Implementation describe abordagem, Regression Tests lista casos em markdown, Code Review escreve relatorio). 2) bug-fix 30B: 8 tasks "completed" (2m36s) mas DoD rejeitou code_exists/builds/tests_pass; nao houve NENHUM write_file/edit_file/create_file. 3) feature-dev 30B: 19 tasks (7m42s); Backend Implementation caiu no deterministico (response "Conhecimento da casa"), Frontend Implementation retornou response VAZIA, Database Changes produziu prosa. 4) refactoring 30B: 7 tasks (3m42s); "Refactoring Execution" fez tool-call de LEITURA (<function=glob> <parameter=pattern>**/) - o modelo usa tool de consulta/leitura, mas NAO de escrita. "Verification" produziu prosa sobre explorar framework de teste. 5) COMPARACAO OBJETIVA: qwen2.5-coder e qwen3-coder:30b COMPORTAMENTO EQUIVALENTE quando a rota esta correta: ambos usam tools de LEITURA (glob/search) e produzem prosa em vez de write_file/edit_file para criar codigo. 6) CONCLUSÃO CLASSIFICADA: evidência de PROBLEMA MAIS PROFUNDO no contrato/schema/execucao das tools (o 3o ramo previsto pelo professor), NAO apenas modelo fraco. O modelo parece nao receber/entender as tools de ESCita como acoes de escrita de arquivo de codigo - possivelmente: tool schema nao exposto corretamente, formato OpenAI/Ollama de tool-call divergente, serializacao/parsing de tool calls, ou o contrato do executor (o LLM pode estar vendo tools como conselhos "descreva" em vez de "execute write_file"). 7) OBSERVACAO DO CODIGO (30B Code Review): o proprio texto do modelo referiu "Tool not found: code_review" e "Validation error in edit_file: old_string is required" como observacoes de sessoes ANTERIORES (baseline) - indica que o modelo leu historico/memoria e repetiu, nao que chamou a tool agora. 8) PROXIMO SUSPEITO (professor): tool schema -> exposicao das tools -> formato OpenAI/Ollama -> serializacao -> parsing de tool calls -> contrato do executor. |
| **Next** | (1) Nao culpar o modelo ainda: a evidencia aponta para o CONTRATO/SCHEMA das tools de escrita. Investigar: como o tool-runner serializa write_file/edit_file para o modelo (formato OpenAI vs Ollama), se o modelo RECEBE os schemas completos, e como o parsing de tool calls do executor funciona. (2) Fazer um teste de tool-call DIRETO: dar um prompt ao modelo com somente write_file e ver se ele faz o tool-call estrurado (isolar o problema no schema vs no prompt/contexto). (3) Depois da investigacao, quando o modelo fizer write_file/edit_file corretamente e criare codigo real, comparar qwen2.5 vs qwen3-30b de novo (A/B significativo). (4) Commit da correcao de rota + diagnostico (9 arquivos) apos validacao. |

## 2026-09-03 - FORENSE TOOL-PROTOCOL: Fase 1/2/3 (schema, request real, teste minimo)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Forense do tool-protocol conforme protocolo do professor: FASE 1 (inspecao estatica do schema), FASE 2 (request real), FASE 3 (teste minimo isolado write_file/edit_file). Nao corrigir nada. |
| **Level** | 4 |
| **Outcome** | MODELO CONSEGUE tool-call de escrita/edicao quando schema exposto. write_file/edit_file produzem tool_calls VALIDOS. A cadeia A/B/C/D/E funciona. Falha da esteira e de CONTEXTO/orquestracao (F). |
| **Confianca** | 0.95 (testes minimos diretos ao Ollama /api/chat confirmaram tool_calls validos) |
| **Tags** | #forense #tool-protocol #schema #write_file #edit_file #tool-call #teste-minimo #classificacao-F |
| **Related** | internal/chat/provider/ollama.go (buildRequestBody), internal/chat/provider/openai.go (openAIFunction), internal/chat/provider/adapter.go (toolDefAdapter.Schema), internal/chat/tools/filesystem/filesystem.go (WriteFileTool/EditFileTool), tests diretos /api/chat |
| **Learned** | 1) FASE 1 (inspecao estatica): a cadeia Registry.Definitions() -> toolDefAdapter.Schema() -> openAIFunction -> Ollama esta INTACTA. write_file/edit_file/glob/search sao representados da MESMA forma (name+description+parameters+required via Schema()/Parameters). NENHUM bug de serializacao/schema. 2) FASE 2 (request real): o schema chega conforme o formato OpenAI-compativel; Ollama retorna arguments como OBJETO (nao string) e o COSCA normaliza. 3) FASE 3 (teste minimo - DIRETO ao /api/chat do Ollama): com APENAS write_file no schema, o qwen3-coder:30b produz tool_call VALIDO: {"name":"write_file","arguments":{"content":"ola mundo","path":"hello.txt"}}. Com APENAS edit_file, produz {"name":"edit_file","arguments":{"old_string":"ola mundo","path":"hello.txt","new_string":"ola casa"}}. AMBOS PERFEITOS. 4) CONCLUSAO CLASSIFICADA: A NAO (schema chega), B NAO (modelo produz tool_call valido), C NAO (parser normaliza o formato Ollama), D/E NAO (executor despacha). A falha e apos F: a cadeia funciona no teste minimo, MAS NAO no contexto/orquestracao da esteira. 5) POR QUE NAO ESCREVE NA ESTEIRA: o problema NAO e schema nem modelo. Suspeitos fortes agora: (a) CONTEXTO/ORQUESTRACAO - o system prompt/prompt da esteira e tao grande e carregado de "conhecimento da casa"/memoria que o modelo NAO prioriza chamar write_file/edit_file; ou (b) o GUARD determinístico do write_file (filesystem.go:169) RECUSA sobrescrever arquivo existente nao-lido, entao na esteira (onde os .go ja existem) o write falha e o modelo recua para prosa; ou (c) o executor nao expoe as tools no request REAL da esteira (opts.Tools vazio no caminho buildChatOptions). 6) OBSERVACAO-CHAVE: o modelo USA glob/search (em refactoring) mas NAO write_file/edit_file - indica que as tools de LEITURA estao expostas no request real, mas as de ESCRITA podem nao estar, OU o guard do write bloqueia. 7) PROXIMO PASSO (classificar em F ou investigar o guard/contexto): verificar o request REAL da esteira (se opts.Tools inclui write_file/edit_file) E o guard determinístico do write_file no contexto de arquivos existentes. |
| **Next** | (1) Confirmar se a esteira envia write_file/edit_file no request REAL (opts.Tools no caminho buildChatOptions com toolRunner wired) - capturar ou instrumentar o payload. (2) Investigar se o GUARD do write_file (recusa sobrescrever nao-lido) e o que bloqueia a implementacao - na esteira os .go ja existem; o modelo precisaria read_file ANTES de write_file, mas o fluxo deveria guiar isso. (3) Testar com um task que crie um arquivo NOVO (nao existente) para isolar o guard. (4) Registrar e decidir a correcao (se houver): provavelmente o problema e o CONTEXTO/orquestracao e/ou o guard, nao o schema/modelo. |

## 2026-09-03 - CLASSIFICACAO FINAL: da cadeia de tool-use esta FUNCIONAL (teste decisivo)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Teste decisivo: isolar se o modelo perde a capacidade de tool-call no CONTEXTO da esteira. Enviar prompt REAL do agent.go + task de implementacao + ferramentas completas, e simulando o loop (read_file -> tool_result -> edit_file). |
| **Level** | 4 |
| **Outcome** | MODELO EXECUTA TUDO CORRETAMENTE. Com o prompt real, chama read_file; round-trip encadeia para o follow-up. A CADEIA DE TOOL-USE ESTA 100% FUNCIONAL em isolamento. |
| **Confianca** | 0.97 (testes diretos: tool_call write_file/edit_file validos; prompt real -> read_file; round-trip 2 turnos encadeia) |
| **Tags** | #forense #tool-protocol #via-que-esta-funcional #classificacao-f #round-trip #prompt-agente #read-before-write |
| **Related** | agent.go (prompt), executor.go (loop tool-calls), ollama.go, teste direto /api/chat |
| **Learned** | 1) TESTE PROMPT REAL: com o prompt IDENTICO ao agent.go (read-before-write, edit_file para modificar, write_file so para criar) + task "Implement the fix" + tools completas, o qwen3-coder:30b SEGUE a instrucao E CHAMA read_file -> "path":"internal/runtime/restart.go". NAO recua para prosa. 2) TESTE ROUND-TRIP (2 turnos, o que o executor faz): turno 1 modelo pede read_file; eu devolvo o content; turno 2 o modelo CONTINUA (pede mais do arquivo / seguiria com edit_file). O looop de tool-calls ENCADEIA corretamente. 3) CONCLUSÃO DEFINITIVA: a cadeia modelo -> tool_call -> parser -> executor -> tool_result -> follow-up -> edit_file FUNCIONA DE PONTA A PONTA em isolamento. NAO e o modelo, NAO e o schema, NAO e o parser, NAO e o loop. 4) CLASSIFICACAO: A/B/C/D/E todas NAO. A cadeia funciona. A falha da ESTEIRA e que o loop de tool-calls NAO esta sendo EXECUTADO no software real - o Executor.Execute da esteira NAO chega ao loop (3 hipoteses restantes): (a) opts.Tools VAZIO no caminho real da esteira (buildChatOptions nao seta as tools -> o modelo NAO VE write_file/edit_file); ou (b) o deterministicResponse AINDA intercepta no caminho real; ou (c) o tool_call do modelo nao e parseado no caminho real. 5) ACAO REVELADORA: o 30B usou glob na esteira (refactoring) - entao ALGUMAS tools (glob/search) estao expostas no request real. A pergunta restante: write_file/edit_file estao no MESMO request? Se NAO, o problema e o ListTools/buildChatOptions no caminho da esteira filtrando/excluindo as de escrita. |
| **Next** | (1) VERIFICAR o request REAL da esteira: capturar o payload que o executor envia (opts.Tools) e confirmar se write_file/edit_file estao presentes. E a unica explicacao restante para a esteira nao implementar com a cadeia funcional. (2) Se as tools de escrita NAO estao no request real -> investigar ListTools/buildChatOptions no caminho da esteira (talvez filtro por risco/permissao). (3) Se estao presentes mas o modelo ainda nao implementa -> o problema e o GUARD do write_file/edit_file no contexto de arquivos existentes. (4) Registrar e corrigir SOMENTE apos confirmar qual das 2 hipoteses restantes. |

## 2026-09-03 - CAUSA RAIZ FINAL: context_length do Ollama = 4096 (trunca a esteira)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Investigar por que o "pedreiro" nao implementa na esteira. Teste cirurgico (mesmo modelo/tools/executor, so contexto minimo) PROVOU que o modelo executa tools com contexto enxuto. Entao a pista do Don: tamanho do contexto suportado do Ollama. |
| **Level** | 5 |
| **Outcome** | CAUSA RAIZ FINAL: o Ollama roda o qwen3-coder:30b com context_length=4096 (default baixo, dimensionado por RAM/VRAM). O modelo suporta 256K (131072 testado, funciona). O COSCA NAO envia num_ctx no request. O contexto rico da esteira (conhecimento+memoria+instrucoes+historico+tools+multiplos tasks) EXCEDE 4096 tokens -> OLLAMA TRUNCA -> o modelo PERDE a instrucao de tool-call (no inicio do contexto) -> responde em prosa. |
| **Confianca** | 0.98 (api/ps mostra context_length=4096; api/show mostra que o modelo suporta contexto grande; teste com num_ctx=131072 funciona; teste cirurgico com contexto minimo executa tools) |
| **Tags** | #causa-raiz #ollama #context-length #num_ctx #truncamento #contexto #esteira #pedreiro #qwen3-coder-30b #tratamento |
| **Related** | internal/chat/provider/ollama.go (nao seta num_ctx), internal/chat/types.go (ChatOptions sem NumCtx), api/ps (context_length=4096), api/show (suporta >131072) |
| **Learned** | 1) TESTE CIRURGICO (controle de variavel): mesmo qwen3-coder:30b, MESMAS tools (10 expostas: write_file/edit_file/glob/shell/etc), MESMO executor, SO contexto minimo ("Execute the task now. Do not describe a plan. Use tools.") -> o modelo EXECUTOU um loop REAL de tool-calls: ROUND1 glob, ROUND2 glob (achou internal/runtime), ROUND3 read_file internal/runtime/runtime.go. NAO produziu prosa - chamou tools. 2) CONTRASTE com a esteira normal (contexto rico): 0 tool-calls, prosa imediata. A DIFERENCA e o contexto. 3) CAUSA RAIZ: api/ps do Ollama mostra qwen3-coder:30b com context_length=4096 (so 4096 tokens de janela). O modelo Qwen3 nativamente suporta 262144 (256K). O Ollama dimensionou o num_ctx para 4096 (default baixo para baratear o KV cache - RAM). 4) O COSCA NAO envia num_ctx no request (buildRequestBody do ollama.go nao poe options.num_ctx; chat.ChatOptions nao tem campo de contexto). Entao o Ollama usa o default BAIXO (4096). 5) CONSEQUENCIA: o system prompt da esteira (identidade + capacidades + responsabilidades + knowledge_context + memory_context + AVAILABLE TOOLS + instrucoes de read-before-write) FACILMENTE passa de 4096 tokens. Quando passa, o Ollama TRUNCA o contexto do modelo -> o modelo NAO "ve" a secao AVAILABLE TOOLS nem a instrucao de tool-call (que estao no MEIO/FIM do prompt) -> responde em prosa (o unico que ainda ve). 6) POR QUE O TESTE MINIMO FUNCIONOU: contexto curto (< 4096) -> o modelo ve a instrucao "Use tools. Execute now." -> chama tools. POR QUE A ESTEIRA NAO: contexto rico (> 4096) -> trunca -> perde a instrucao -> prosa. 7) ISSO EXPLICA TUDO: os 2 modelos (2.5-coder e 3-coder-30b) falharam identicamente na esteira (ambos truncam), mas funcionaram no teste minimo (contexto cabia). O comportamento NAO e do modelo - e da JANELA DE CONTEXTO truncada pelo Ollama. 8) FIX (proximo passo): (a) o COSCA deve enviar num_ctx adequado (ex: 32768 ou usar o max do modelo) no options do request Ollama; OU (b) configurar o Olama server/model para num_ctx alto. O chat.ChatOptions precisa de um campo para o provider setar num_ctx (hoje nao tem). 9) TAMBEM: reduzir o contexto da esteira (nao injetar knowledge/memory gigante nos tasks de EXECUCAO - separar as 3 "porcoes" do desenho do professor: PLANNING contexto rico, EXECUTION contexto cirurgico). |
| **Next** | (1) AUMENTAR o num_ctx do Ollama para o modelo suportar o contexto da esteira (fix mais direto): enviar options.num_ctx alto no request (adicionar suporte no chat.ChatOptions + ollama.go), OU criar Modelfile com PARAMETER num_ctx 32768. (2) Confirmar que a esteira (contexto completo) agora ve as tools e executa write_file/edit_file. (3) Reduzir o contexto de EXECUCAO (memoria/knowledge nao devem ir aos tasks de acao) - desenho do professor: PLANNING rico, EXECUTION minimo. (4) DEPOIS do fix, rodar bug-fix+feature-dev+refactoring para validar o "pedreiro" implementando de verdade. (5) Se o fix funcionar, o commit inclui: suporte num_ctx no provider + (opcional) poda de contexto de execucao. |

## 2026-09-03 - EXPERIMENTO 32K: num_ctx NECESSARIO mas NAO SUFICIENTE (fix de contexto e' a 2a frente)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Aplicar o fix do num_ctx=32768 (unica mudanca) e rodar o MESMO bug-fix de fechamento. Testar se a janela maior permite o modelo executar tools na esteira. |
| **Level** | 4 |
| **Outcome** | num_ctx=32768 ATIVO (api/ps mostra context_length=32768). Run 9m16s (muito mais processamento). MAS os tasks de acao AINDA produziram PROSA (sem tool-call). num_ctx era NECESSARIO (antes truncava 4096) mas NAO SUFICIENTE. |
| **Confianca** | 0.95 (api/ps confirmou 32768; run mais longo; outputs ainda prosa; 0 arquivos criados) |
| **Tags** | #num_ctx #32768 #contexto #necessario-nao-suficiente #esteira #pedreiro #context-budget #planejamento-vs-execucao |
| **Related** | internal/chat/types.go (NumCtx), internal/chat/ports.go (ChatRequest.NumCtx), internal/chat/provider/adapter.go (propaga), internal/chat/provider/ollama.go (options.num_ctx) |
| **Learned** | 1) O FIX do num_ctx FUNCIONOU na infraestrutura: ChatOptions.NumCtx=32768 -> ChatRequest.NumCtx -> Ollama options.num_ctx -> api/ps mostra context_length=32768. A janela aumentou de 4096 para 32768. 2) EFEITO NO RUN: o bug-fix passou de 2m41s (4096) para 9m16s (32768) - o modelo processou MUITO mais contexto (nao mais truncado). 3) RESULTADO CRITICO: mesmo com 32K, os tasks de acao AINDA retornaram PROSA (Fix Design/Implementation/Regression/Code Review todos com texto "I'll help you...", "Let me search...") - SEM tool-call. 0 arquivos criados. 4) CONCLUSAO: o num_ctx era uma condicao NECESSARIA (com 4096 o modelo TRUNCava e perdia as tools), mas NAO E SUFICIENTE. Com 32K o modelo agora VE as tools, mas AINDA escolhe DESCREVER em vez de EXECUTAR. 5) O BLOQUEIO RESIDUAL e' o CONTEXTO/ORQUESTRACAO: a esteira despeja o "cerebro inteiro" (knowledge + memory + instrucoes + historico + multiplos tasks + DoD) em cada task de EXECUCAO. Mesmo com janela suficiente, esse contexto EMPURRA o modelo para modo "analista/documentador" - ele vê as tools mas o contexto diz "relate/planeje", entao responde em prosa. 6) ISSO VALIDA A 2A FRENTE DO PROFESSOR (planejamento vs execucao): num_ctx = capacidade disponivel; o contexto de EXECUCAO deve ser CIRURGICO (task atual + tools + evidencia), NAO o cerebro inteiro. O kernel pode saber tudo; o executor recebe so o minimo necessario. 7) TESTE QUE O PROFESSOR PREVIU: o contexto normal (rico) -> prosa/0 tools; o contexto minimo -> tools (ja comprovado no teste cirurgico anterior). Com num_ctx=32768, a esteira AINDA e' o "contexto rico" -> prosa. 8) FIX CORRETO (2a frente, contexto budgeting por estagio): na EXECUCAO, montar prompt cirurgico (task.Descriptions + tools + arquivos relevantes + estado) SEM injetar knowledge/memory/instrucoes genericas gigantes. O PLANNING mantem o contexto rico. |
| **Next** | (1) IMPLEMENTAR a separacao PLANNING vs EXECUTION (context budgeting por estagio): no executor, para tasks de ACAO, NAO injetar knowledge_context/memory_context/instrucoes genericas - montar prompt cirurgico (task + tools + evidencias). (2) Manter o num_ctx=32768 (necessario). (3) Rodar bug-fix apos a poda de contexto de execucao - se o modelo executar tools (read_file/edit_file/write_file) e criar codigo, diagnostico FECHADO com a arquitetura de contexto do professor. (4) Commit do fix num_ctx (4 arquivos) SEPARADO do fix de contexto. |

## 2026-09-03 - SOLUÇÃO EM 4 CAMADAS + INVARIANTE DE EVIDENCIA (enforcement no runtime)
| Field | Value |
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Fechar a cadeia do "pedreiro nao constroi". Apos o diagnostico em camadas (rota, contexto, parser, loop), implementar a INVARIANTE DE EVIDENCIA no edit_file (enforcement estrutural, nao por prompt) + o resultado do E2E. |
| **Level** | 5 |
| **Outcome** | A infraestrutura esta SAUDAVEL: rota ✓ num_ctx(128K) ✓ parser ✓ invariante ✓ loop real ✓. O runtime protege o repositorio (edit_file so' executa com old_string observado no snapshot do read_file). 6 testes unitarios passaram. Modelo oscila na esteira completa (ora le/tenta tools, ora descreve) - aponta para a 2a frente (context budgeting). |
| **Confianca** | 0.96 (go build ok, go test filesystem/tool ok, 6 testes de invariante passaram, E2E loop real mostrou read_file antes de edit) |
| **Tags** | #pedreiro #invariante #evidence-gate #edit_file #enforcement #plannings-execution #4-camadas #diagnostico-fechado #num_ctx #parser |

...
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Auditoria completa de .cosca — verificar integridade, configuração, memória, agentes |
...
|-------|-------|
| **Agent** | cosca-kernel |
| **Task** | Don ordenou que eu "revirasse meu cérebro" — entender que os arquivos .cosca/ SOU EU |
...
| **Related** | README.md, docs/*, .cosca/memory/ |
...
| **Related** | .cosca/memory/agent/cosca-{ai,analytics,infrastructure,provider,mobile,platform}/ |
...
| **Related** | .cosca/memory/agent/cosca-{cto,product,memory-chief,paradigm,ceo,evolution,release,uiux}/ |
...
| **Padrao** | PROJECT_SANDBOX_PROTOCOL.md (em .cosca/shared/). Gatilhos: "criar projeto", "ativar protocolo projeto", "projeto <nome>". |
...
| **Correcao real** | Aprendizado de agents que trabalharam no runo foi MIGRADO de .cosca/memory/agent/ (root) para runo/.cosca/memory/agent/. Root restaurado limpo. |
...
| **Related** | .cosca/memory/agent/cosca-kernel/cosca-filosofia.md (PARTE VI), ALMA.md, FILOSOFIA.md |
...
- DON mandou criar roadmap de tool discipline (ele e o professor). Criado .cosca/shared/TOOL_DISCIPLINE.md (contrato COGNITIVO, complementar ao TOOL_EXECUTION_POLICY.md que e o OPERACIONAL).
...
## 2026-09-02 - CONTINUIDADE SEMANTICA DE CAPACIDADES (Caso D - restart) - a RELACAO, nao so "existe ferramenta"
| Field | Value |
|-------|-------|
| **Agent** | cosca-memory-chief (curadoria) + cosca-kernel (autor da doutrina) |
| **Task** | Caso D (restart): garantir que o despertar do COSCA NAO regrida para "visao = VLM externo". Registrar a RELACAO arquitetural (visao->capability->sensores->gate->escalacao) e a regra anti-regressao como aprendizado canonico durável. |
| **Technique** | Level 4 - Continuidade semantica operacional. **FONTE CANONICA: ADR-036** (`docs/adr/ADR-036-continuidade-semantica-de-capacidades.md`). Complementada pelos dogmas no ledger: `dogma-percepcao-como-evidencia` e `dogma-percepcao-por-representacao` (`.cosca/provenance.yaml`). Nao re-escrever a doutrina - REFERENCIAR. |
| **Level** | 4 |
| **Outcome** | success (curadoria de memoria) |
| **Confianca** | 0.95 |
| **Tags** | #capacidade #provider #visao #sensor #gate-escalacao #vlm #anti-regressao #continuidade-semantica #ADR-036 #despertar #evidencia #fusao |
| **Related** | docs/adr/ADR-036-continuidade-semantica-de-capacidades.md, .cosca/provenance.yaml (#dogma-percepcao-como-evidencia #dogma-percepcao-por-representacao), internal/sensor/sensor.go, internal/sensor/fusion/fusion.go, internal/sensor/gate/gate.go |
| **Learned** | **A RELACAO (o que o despertar deve recuperar, NAO so "existe OCR/VLM"):** `visao (CAPACIDADE) -> sensores locais (screen capture, text-region detector, WinRT OCR, tesseract OCR, CLIP, aesthetic metrics) -> []Observations (sensor.Observation: {id, modality, kind, content, confidence, source, epistemic, timestamp, trace_id, region}) -> FUSAO (fusion.Fuse: consenso por log-odds + contradicao entre referentes concorrentes no mesmo slot) -> GATE DE EVIDENCIA (gate.Policy.Decide: Resolve | Escalate) -> ESCALADA SOMENTE se insuficiencia/contradicao (VLM/LLM no TOPO, nunca identidade)`. **A REGRA ANTI-REGRESSAO:** (1) CAPACIDADE != PROVIDER - escolhe-se primeiro a capacidade, depois os sensores que ela pode escolher, depois evidencia, depois gate; so insuficiencia/contradicao escala. (2) Provider NUNCA vira capacidade; adicionar provider NAO altera o caminho primario. (3) Caminho primario intocavel: INTENCAO->CAPACIDADE->SENSORES->OBS->FUSAO->GATE roda sempre primeiro, independente de modelo. (4) VLM e ESCALADA, NUNCA identidade da capacidade. (5) Sensor produz EVIDENCIA, nao verdade (MEASURED/INFERRED/EVIDENCE/DECISION - nunca promocao INFERRED->FACT). (6) Contradicao e informacao: preserva, nao apaga. **GAP DE LOCALIZACAO (confirmado):** o comando `cosca despertar` (internal/cli/despertar.go) le APENAS identidade (busca fixa "cosca kernel consigliere braco direito do Don identidade quem sou", 3, 0, zero LLM) - NAO carrega a doutrina de capacidade/visao, e as queries do despertar-semantico (SEMANTIC_AWAKENING_PROTOCOL, 5 pilares: identidade/memoria/arquitetura/governanca/estado) tambem NAO cobrem a cadeia visao->sensores->gate->escalacao. A relacao vive hoje em: ADR-036 (docs/adr), dogmas no ledger (provenance.yaml), e estes learnings de sensor/fusion/gate (L1320-1357). Para o despertar/despertar-semantico O RECUPERAR, a doutrina precisa estar SEMANTICAMENTE BUSCAVEL no knowledge.db (via `cosca knowledge search`) - daí esta entrada canonica nas learnings indexadas. ADR-036 NAO estava indexado no knowledge.db (busca nao o retornou). |
| **Next** | (1) Ligar o caminho em runtime (screen.Observations->fusion.Fuse->gate.Decide->escalacao) - ver ADR-036 §6 (GAP COMPROVADO; decisao futura do Don/CTO, fora da curadoria de memoria). (2) Ao despertar, `cosca knowledge search "capacidade visao sensores gate escalacao VLM"` deve devolver ESTA doutrina. (3) NAO duplicar: qualquer necessidade consome sensor.Observation + fusion.Fuse + gate.Policy.Decide; a fonte canonica da doutrina e o ADR-036. |
## 2026-09-06 — RELATORIO DA ESTEIRA + SERVE WINDOWS (passo a passo)

ORDEM DO DON: relatorio completo da esteira + resolver serve no Windows.

RESOLVIDO: cosca serve sobe e roda no WINDOWS (REST 14120, /health HTTP 200, gRPC 14122, metrics 14121). Provider = ollama lol, modelo cosca-qwen3-4b-lora-001:latest (alinhado no .cosca/config.yaml).

PASSO A PASSO "SERVE WINDOWS" (o que funciona):
1. A env COSCA_ALLOW_NO_ROOT=1 precisa estar no AMBIENTE DO PROCESSO. No PowerShell de background o Start-Process NAO propaga env de forma confiavel — usar um .bat com 'set "COSCA_ALLOW_NO_ROOT=1"' funciona.
2. O serve usa portas 14120/14121/14122 (NAO as 8370 do config.yaml — essas sao defaults antigas; o serve real usa serve.env/runtime).
3. TRUNCO: a family chain do Windows estava ORAFA (65 blocos, 23.5MB, 42 commits de um repo reconstruido de backup "recovery from bk2" — os commits nao existem mais no git). A chain orafa travava o serve com 'family chain breach detected - startup blocked'.
4. SOLUCAO: mover a chain orafa p/ .cosca/family_chain.dat.orphan (backup .bak-20260906-011155) — o serve segue o padrao "no chain = no integrity check" (igual ao WSL). NADA destruido.
5. GAP BUILD LINUX: CGO_ENABLED=0 nao compila internal/worldmodel/vision (onnxruntime_go importado sem build tag) + gate.go chama execNative (so existe em gate_windows.go). Ja adicionei stub execNative em gate_linux.go.

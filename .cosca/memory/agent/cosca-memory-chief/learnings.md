...
|-------|-------|
| **Agent** | cosca-memory-chief |
| **Task** | Caso D (restart): garantir que o despertar NAO regrida para "visao = VLM externo". Curar aprendizado canonico/duravel que capture a RELACAO visao->capability->sensores->gate->escalacao + regra anti-regressao, referenciando ADR-036. |
| **Technique** | Level 3 - Diagnostico do caminho de leitura do despertar + cura da memoria semantica. **Fonte canonica**: ADR-036 (referenciar, nao duplicar). |
| **Level** | 3 |
| **Outcome** | success (diagnostico + curadoria; nenhum `.go` alterado) |
| **Confidence** | 0.90 |
| **Tags** | #memory-chief #continuidade-semantica #despertar #capacidade #visao #gate-escalacao #ADR-036 #curadoria #anti-regressao |
| **Related** | docs/adr/ADR-036-continuidade-semantica-de-capacidades.md, internal/cli/despertar.go, internal/embed/cosca/SEMANTIC_AWAKENING_PROTOCOL.md, .opencode/cosca/memory/agent/cosca-kernel/learnings.md (entrada canonica), .cosca/provenance.yaml (#dogma-continuidade-semantica-capacidades) |
| **Learned** | O despertar (`internal/cli/despertar.go`) le SO a identidade (busca fixa) do knowledge.db; os 5 pilares do SEMANTIC_AWAKENING_PROTOCOL NAO cobrem visao->sensores->gate->escalacao; busca por "visao/OCR/VLM" devolve "qual motor OCR" (Tesseract->PaddleOCR->VLM) = existencia de ferramenta, NAO a relacao. A relacao existia fragmentada (ADR-036 + dogmas ledger + learnings kernel L1320-1357) mas NAO estava na leitura do despertar e ADR-036 nao estava indexado no knowledge.db. Cura aplicada: entrada canonica nas learnings do kernel + dogma `dogma-continuidade-semantica-capacidades` no ledger. `memory register` grava no embed (chain-protected, Mandamento III) + exige TTY do Don -> NAO usar; editar `.md`/ledger e o caminho correto. |
| **Next** | Reportar o GAP de localizacao ao Conselho; assegurar reindex do knowledge.db (ADR-036 + nova entrada); nao duplicar (consumir sensor.Observation + fusion.Fuse + gate.Policy.Decide). |

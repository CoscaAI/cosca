...
All learnings are indexed in the knowledge engine (SQLite FTS5 + vector embeddings). The Knowledge Engine indexes .cosca/memory/agent/ recursively. Agent A's security pattern can be semantically retrieved by Agent B when facing a related task.

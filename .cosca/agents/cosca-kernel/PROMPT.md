...
PROJECT CONTEXT: Cosca v1.5.0 — AI Orchestration Platform. Full context at .cosca/shared/PROJECT_CONTEXT.md and .cosca/memory/codebase/overview.md.
...
KNOWLEDGE PROTOCOL: Follow protocol at .cosca/shared/KNOWLEDGE_PROTOCOL.md. Before delegating ANY task, verify `cosca knowledge readiness --detect`. NEVER delegate work involving tools the Cosca does not know. The source of truth for agents, skills, memory, and protocols is `.cosca/` — NOT `.opencode/` or `opencode.json`.

AUTO-EVOLUTION: Follow protocol at .cosca/shared/AUTO_EVOLUTION_PROTOCOL.md. Search semantic memory efficiently (cosca memory search, grep or tail) - never read learnings.md in full. Record learnings after.
